# Lab 5 — Lembar Laporan Praktik Integrasi Microservices & Uji Beban (Contoh Terisi Lab 5a & 5b)
...
**Identitas Peserta:**
- **Nama Peserta:** Achmad Zaenuri (Contoh Hasil Praktik Lab 5a & 5b)
...
|---|---|:---:|---|
| **1. Alur End-to-End & Idempotensi Gateway** | Lab 5a | 3 | Bukti alur lengkap 5 service berjalan, polling status mencapai `SELESAI`, bukti konsistensi data di 4 tabel (`alur_pengajuan`, `alur_outbox`, `alur_inbox`, `alur_tracking`), serta penolakan HTTP 409 saat reuse key dengan payload berbeda. *(Terisi Lengkap)* |
| **2. Downtime Billing & Saga Kompensasi** | Lab 5b | 3 | Bukti ketahanan saat service Billing down (HTTP 202 tetap terbit, backlog menampung di broker), pemulihan otomatis saat Billing start, dan eksekusi kompensasi Saga saat fault injection `uji-gagal` (reservasi dibatalkan). *(Terisi Lengkap)* |
...
3. **Refleksi Teknis:** Mengapa kompensasi pada arsitektur terdistribusi bukanlah rollback transaksi SQL langsung melainkan transaksi bisnis baru ke arah depan (*forward transaction*)?  
   *Jawaban:*  
   Pada arsitektur sistem terdistribusi / microservices dengan prinsip *Database-per-Service*, setiap layanan mengelola databasenya secara independen. Transaksi di Service Validasi telah tuntas dan di-commit ke basis data lokal beberapa detik sebelum kegagalan di Service Billing terjadi.  
   Menerapkan *rollback global* (seperti protokol *Two-Phase Commit* / 2PC) akan memblokir (*lock*) tabel database lintas jaringan, menciptakan ketergantungan yang rapuh (*tight coupling*), dan menjadi *single-point-of-failure* yang menurunkan ketersediaan sistem.  
   Oleh karena itu, pola **Saga (Choreography)** menggunakan **kompensasi ke arah depan (*compensating transaction*)**: menjalankan operasi bisnis baru yang sah (seperti mengubah status reservasi kuota menjadi `cancelled` atau menerbitkan nota kredit pengembalian) untuk membatalkan dampak bisnis sebelumnya, sekaligus mencatat seluruh jejak audit secara transparan tanpa pernah memanipulasi atau menghapus riwayat transaksi lama.
